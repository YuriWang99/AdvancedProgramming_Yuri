...
public class PenguinAreaTest : MonoBehaviour
{
    public PenguinAgentTest penguinAgent;

    public GameObject penguinBaby;

    public TextMeshPro cumulativeRewardText;

    public FishTest fishPrefab;
    ...
    private void PlacePenguin()
    {
        ...
        penguinBaby.transform.rotation = Quaternion.Euler(0f, UnityEngine.Random.RandomRange(0f,360f), 0f);
    ...
    private void SpawnFish(int count, float fishSpeed)
    {
        for (int i = 0; i < count; i++)
        {
            ...
            fishObject.transform.position = ChooseRandomPosition(transform.position, 100f, 260f, 2, 13f) + Vector3.up * .5f;
            fishObject.transform.rotation = Quaternion.Euler(0f, UnityEngine.Random.Range(0, 360f), 0f);

            fishObject.transform.SetParent(transform);

            fishList.Add(fishObject);

            fishObject.GetComponent<FishTest>().fishSpeed = fishSpeed;
        }
    }
